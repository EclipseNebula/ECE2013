package com.remainsoftware.ece2013.nebula.snippets.gantt;
...
import org.eclipse.nebula.widgets.ganttchart.VerticalDragModes;
...
public class GanttTester {
	...
	private Preferences _prefs = Preferences
			.systemNodeForPackage(GanttTester.class);

	private static final String KEY = "prefKey";
	public static final String ID = "ganttView";
	...
	@PostConstruct
	public void ganttTester(Composite parent) {

		parent.setLayout(new FillLayout());

		final SashForm sfVSplit = new SashForm(parent, SWT.VERTICAL);
		...
		final ScrolledComposite sc = new ScrolledComposite(rightForm,
		...
		rightComposite.addListener(SWT.Resize, new Listener() {
			...
			public void handleEvent(final Event event) {
				sc.setMinSize(rightComposite.computeSize(SWT.DEFAULT,
						SWT.DEFAULT));
			}
		...
		// parent.setLocation(new Point(m.getClientArea().x, 0));
		 ...
		 * display.sleep(); } } display.removeListener(SWT.KeyDown,
		 * _undoRedoListener);
		 * 
		 * shell.dispose();
		 */
	...
	private Composite createEventLogTab(final Composite parent) {
		...
		_tEventLog = new Table(group, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL
	...
	private Composite createAdvancedTab(final Composite parent) {
		...
		_bEnableAutoScroll
				.setToolTipText("Causes chart to auto scroll when using DND. On by default.");
		...
		_bEventResizing
				.setToolTipText("Allows for resizing of events. On by default.");
		...
		_bEventDND
				.setToolTipText("Allows for dragging of events. On by default.");
		...
		_bAdjustForLetters
				.setToolTipText("When letters are drawn anywhere in the header they are auto-adjusted to fit in nicely. On by default.");
		...
		_bConnectionLineStyle
				.setToolTipText("How the connecting line is drawn. Default is MS Project Style");
		...
		_bShowBoldScopeText
				.setToolTipText("Scope Text is Bold. Default is on.");
		...
		_bShowGradientEventBars
				.setToolTipText("Gradient Events On/Off. Default is on. (If off only 1 color is used)");
		...
		_bShowOnlyDependenciesForSelectedItems
				.setText("Show Only Dependencies for Selected Items");
		_bShowOnlyDependenciesForSelectedItems
				.setToolTipText("Shows only dependency arrows when items are selected and the selected event has connections/dependencies to other events. Default is off.");
		_bShowOnlyDependenciesForSelectedItems.setSelection(false);
		_bShowOnlyDependenciesForSelectedItems.setLayoutData(oneRow);
		_bShowOnlyDependenciesForSelectedItems.setData(KEY,
				"bShowOnlyDependenciesForSelectedItems");
		...
		_bShowAdvancedTooltips
				.setToolTipText("Advanced Tooltips on/off. Default is on.");
		...
		_bEnableZooming
				.setToolTipText("Whether chart can be zoomed / zoomed out. Default is on.");
		...
		_bShowZoomLevelBox
				.setToolTipText("Box that shows what zoom level is currently being zoomed to on/off. Default is on.");
		...
		_bZoomToMousePointerDateOnWheelZooming
				.setText("Zoom In/Out To Mouse Pointer");
		_bZoomToMousePointerDateOnWheelZooming
				.setToolTipText("When zooming the chart will try to keep the focus on the date where the mouse pointer is when the zoom happens. Default is on.");
		_bZoomToMousePointerDateOnWheelZooming.setSelection(true);
		_bZoomToMousePointerDateOnWheelZooming.setLayoutData(oneRow);
		_bZoomToMousePointerDateOnWheelZooming.setData(KEY,
				"bZoomToMousePointerDateOnWheelZooming");
		...
		_bAllowBlankAreaDragAndDropToMoveDates
				.setText("Allow Horizontal Blank Area DND to Move Chart");
		_bAllowBlankAreaDragAndDropToMoveDates
				.setToolTipText("When you grab a blank area of the chart and drag it, the chart will move in the drag direction horizontally. Default is on.");
		_bAllowBlankAreaDragAndDropToMoveDates.setSelection(true);
		_bAllowBlankAreaDragAndDropToMoveDates.setLayoutData(oneRow);
		_bAllowBlankAreaDragAndDropToMoveDates.setData(KEY,
				"bAllowBlankAreaDragAndDropToMoveDates");
		...
		_bAllowVerticalBlankDnd
				.setText("Allow Vertical Blank Area DND to Move Chart)");
		_bAllowVerticalBlankDnd
				.setToolTipText("When you grab a blank area of the chart and drag it, the chart will move in the drag direction vertically. Default is off.");
		...
		_bFlipBlankAreaDragDirection
				.setToolTipText("This will invert the X-axis of the direction the chart is scrolling when blank-area drag and dropping (natural to most people). Default is on.");
		...
		_bDrawSelectionMarkerAroundSelectedEvent
				.setText("Draw Selection Marker Around Selected Events");
		_bDrawSelectionMarkerAroundSelectedEvent
				.setToolTipText("This will draw a selection marker (dotted line) around selected events. Default is on.");
		_bDrawSelectionMarkerAroundSelectedEvent.setSelection(true);
		_bDrawSelectionMarkerAroundSelectedEvent.setLayoutData(oneRow);
		_bDrawSelectionMarkerAroundSelectedEvent.setData(KEY,
				"bDrawSelectionMarkerAroundSelectedEvent");
		...
		_bAllowCheckpointResizing
				.setToolTipText("This will allow for resizing of events marked as checkpoints. Default is false.");
		...
		_bStartCalendarOnFirstDayOfWeek
				.setText("Start Calendar on First Day of Week");
		_bStartCalendarOnFirstDayOfWeek
				.setToolTipText("This will force calendar to start on the first day of the week of whatever the root calendar is. Default is false.");
		_bStartCalendarOnFirstDayOfWeek.setSelection(false);
		_bStartCalendarOnFirstDayOfWeek.setLayoutData(oneRow);
		_bStartCalendarOnFirstDayOfWeek.setData(KEY,
				"bStartCalendarOnFirstDayOfWeek");
		...
		_bDrawFullPercentageBar
				.setToolTipText("Percentage bar is filled in beyond the percentage complete value (assuming it's less than 100%) to fill out the entire event. Default is on.");
		...
		_bDrawLockedDateMarks
				.setToolTipText("Whether locked date-range events (min / max dates) draw a bounding area or markers showing where the span starts/ends. Default is on.");
		...
		_bShowDateTipsOnScrolling
				.setToolTipText("Whether date tips (tooltips) are shown when scrolling around to indicate what dates the user is viewing. Default is on.");
		...
		_bScaleImageToDay
				.setToolTipText("Whether images should be scaled to the width of one day or if they can exceed one day. Default is on.");
		...
		_bAllowArrowKeysToMoveChart
				.setToolTipText("Whether arrow keys can scroll the chart left/right/up/down (like navigating with scrollbars). Default is off.");
		...
		_bCreateSpecialRangesWithAllowNoEvents
				.setText("Don't Allow Events on Special Date Ranges");
		_bCreateSpecialRangesWithAllowNoEvents
				.setToolTipText("Whether special date ranges will not allow drag and drop or resizing over them. Default is off.");
		_bCreateSpecialRangesWithAllowNoEvents.setSelection(false);
		_bCreateSpecialRangesWithAllowNoEvents.setLayoutData(oneRow);
		_bCreateSpecialRangesWithAllowNoEvents.setData(KEY,
				"bCreateSpecialRangesWithAllowNoEvents");
	...
	private Composite createCreationTab(final Composite parent) {
		...
		_bIncreaseDates
				.setToolTipText("Automatically increases the date for each subesquently created event");
		...
		_bCreatePlannedDates
				.setToolTipText("Creates planned dates for each event");
		...
		_bDrawHorizontalLines
				.setToolTipText("Draws horizontal lines between events at the location of the height of the row where the event lives");
		...
		_bUseSectionsRight
				.setToolTipText("Creates the section bar on the right");
		...
		_bConnectEvents
				.setText("Connects events up to the number defined in the spinner below");
		...
		_sConnectionCountNumber
				.setToolTipText("Number of events that should be connected\n(Very large numbers is not suggested)");
		...
		_bRandomColors
				.setToolTipText("Uses random colors for connecting events");
		...
		_bRandomEventColors
				.setToolTipText("Uses random colors for filling events");
		...
		_bDNDLimits
				.setToolTipText("Creates limits that dates cannot be resized/dragged beyond");
		...
		_bRandomPercentCompletes
				.setToolTipText("Creates random percent completes from 0 to 100 on each event");
		 ...
		 * bRandomEventHeights
		 * .setText("Random Event Heights (eventHeight to 100)");
		 * bRandomEventHeights.setToolTipText(
		 * "Creates random event heights for each event between the range of [defaultEventHeight] to 100"
		 * );
		 */
		...
		_bSpecialDateRange
				.setToolTipText("Creates one Special Date Range as Example (with random colors)");
		...
		_bRandomRowHeights
				.setToolTipText("Creates random row heights for each event between the range of [defaultEventHeight] to 100");
		...
		_bRandomEventVLoc
				.setToolTipText("Creates random location for each event one of (SWT.TOP, SWT.CENTER, SWT.BOTTOM)");
		...
		_bRandomEventTextHLocation
				.setText("Random Event Horizontal Text Location");
		_bRandomEventTextHLocation
				.setToolTipText("Creates random event Text location for each event one of (SWT.LEFT, SWT.CENTER, SWT.RIGHT)");
		...
		_bRandomEventTextVLocation
				.setText("Random Event Vertical Text Location");
		_bRandomEventTextVLocation
				.setToolTipText("Creates random event Text location for each event one of (SWT.TOP, SWT.CENTER, SWT.BOTTOM)");
		...
		_bRandomRowHeights.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				_bRandomEventTextHLocation.setEnabled(_bRandomRowHeights
						.getSelection());
				_bRandomEventTextVLocation.setEnabled(_bRandomRowHeights
						.getSelection());
			}
		...
		_bLockHeader
				.setToolTipText("Locks the header so that it is always shown regardless of vertical scroll");
		...
		_bMoveOnlyLaterLinkedEvents
				.setText("Move/Resize Only 'Older' Linked Events");
		_bMoveOnlyLaterLinkedEvents
				.setToolTipText("When moving/resizing linked events, this flag ensures that only older events than the ones being dragged are moved/resized");
		...
		_bConnectEvents.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				_sConnectionCountNumber.setEnabled(_bConnectEvents
						.getSelection());
		...
		_timerText.setBackground(Display.getDefault().getSystemColor(
		...
		_undoRedoListener = new Listener() {
			public void handleEvent(final Event event) {
				if ((event.stateMask & SWT.MOD1) != 0
						&& (event.keyCode == 'z' || event.keyCode == 'Z')) {
					_ganttComposite.getUndoRedoManager().undo();
				}
				if ((event.stateMask & SWT.MOD1) != 0
						&& (event.keyCode == 'y' || event.keyCode == 'Y')) {
	...
	private void createButtonClicked() {
		...
		_ganttComposite.getUndoRedoManager().addUndoRedoListener(
				new UndoRedoListenerAdapter() {
		...
		_ganttChart.addGanttEventListener(new IGanttEventListener() {
			public void eventDoubleClicked(final GanttEvent event,
				...
				final Shell shell = new Shell(Display.getDefault(), SWT.TITLE
						| SWT.CLOSE | SWT.MIN | SWT.ON_TOP
						| SWT.APPLICATION_MODAL);
				shell.setLocation(
						Display.getDefault().getBounds().width * 1 / 5, Display
								.getDefault().getBounds().height * 1 / 3);
			...
			public void eventHeaderSelected(Calendar newlySelectedDate,
				...
				eventLog("Header date selected: " + newlySelectedDate.getTime()
						+ ". Now a total of " + allSelectedDates.size()
						+ " header date(s) selected.");
			}

			public void eventMovedToNewSection(GanttEvent event,
					GanttSection oldSection, GanttSection newSection) {
				eventLog("Eveent'" + event + "' was moved from GanttSection '"
						+ oldSection.getName() + "' to '"
						+ newSection.getName() + "'");
			}
			...
			public void eventSelected(GanttEvent event, List allSelectedEvents,
				...
				eventLog("Event '" + event + "' was selected. "
						+ allSelectedEvents.size()
						+ " event(s) are now selected in total.");
			}
			...
			public void eventsDroppedOrResizedOntoUnallowedDateRange(
					List events, GanttSpecialDateRange range) {
				eventLog("Events '" + events
						+ "' were dropped or resized over special range '"
						+ range + "' that does not allow events on its dates");
		...
		if (_bUseSections.getSelection()
				&& (_bUseSectionsLeft.getSelection() || _bUseSectionsRight
						.getSelection())) {
		...
		if (_bSpecialDateRange.getSelection()) {
			...
			final GanttSpecialDateRange range = new GanttSpecialDateRange(
					_ganttChart, start, end);
			range.setFrequency(_bUseDDay.getSelection() ? GanttSpecialDateRange.REPEAT_DDAY
		...
		for (int i = 0; i < numberEvents; i++) {
			...
			if (i != 0) {
				predEvent = ((GanttEvent) _ganttComposite.getEvents()
						.get(i - 1));
			...
			if (_bIncreaseDates.getSelection()) {
				cEndDate.add(Calendar.DATE,
						_bRandomEventLength.getSelection() ? r.nextInt(10) + 1
								: 1);
			}
			...
			if (_bCreatePlannedDates.getSelection()) {
				...
				plannedStart.add(Calendar.DATE, _bRandomEventLength
						.getSelection() ? -(r.nextInt(10) + 1) : -10);
				final Calendar plannedEnd = (Calendar) cEndDate.clone();
				plannedEnd.add(Calendar.DATE, _bRandomEventLength
						.getSelection() ? (r.nextInt(10) + 1) : 10);

				ganttEvent = new GanttEvent(_ganttChart, null, "Event "
						+ (i + 1), plannedStart, plannedEnd, cStartDate,
						cEndDate, 0);
			} else {
				ganttEvent = new GanttEvent(_ganttChart, null, "Event "
						+ (i + 1), cStartDate, cEndDate, 0);
			}
			...
			if (_bUseSections.getSelection()
				...
				if (lastSectionEventCount != 0 && !reachedMax) {
					if (r.nextInt(2) == 1) {
						sectionCount++;
						parent = new GanttSection(_ganttChart, "Section "
								+ sectionCount);
			...
			if (_bRandomEventColors.getSelection()) {
				ganttEvent.setStatusColor(ColorCache.getColor(r.nextInt(255),
						r.nextInt(255), r.nextInt(255)));
				ganttEvent.setGradientStatusColor(ColorCache.getColor(
			...
			if (_bConnectEvents.getSelection()
					&& i < _sConnectionCountNumber.getSelection()) {
				if (_bRandomColors.getSelection()) {
					_ganttComposite.addConnection(predEvent, ganttEvent,
							ColorCache.getColor(r.nextInt(255), r.nextInt(255),
									r.nextInt(255)));
				} else {
					_ganttComposite.addConnection(predEvent, ganttEvent);
				}
		...
		_timerText.setText("Initial chart creation took " + (time2 - time1)
				+ " ms");
	...
	private void createCreateButtons(final Composite parent) {
		...
		_bSaveFull
				.setToolTipText("Saves an image of the chart, the entire chart, and nothing but the chart");
		...
		_bHeavyRedraw.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				_timerText.setText("Heavy redraw took " + (time2 - time1)
						+ " ms");
	...
	private void prefHook(final Control ctrl) {
		...
		if (obj == null) {
			System.err.println("Control " + ctrl
					+ " does not have a key set on it!"); // NOPMD
			return;
		}
		...
		if (ctrl instanceof Spinner) {
			...
			((Button) ctrl).addListener(SWT.Selection, new Listener() {
				public void handleEvent(Event event) {
					_prefs.put(keyName, ((Button) ctrl).getSelection() ? "true"
							: "false");
				}
			});
	...
	private Composite createBottom(final Composite parent) {
		...
		bSelectFirstEvent.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				_ganttComposite.setTopItem((GanttEvent) _ganttComposite
						.getEvents().get(0), SWT.CENTER);
				moveFocus();
			}
		});
		...
		bSelectLastEvent.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				_ganttComposite.setTopItem((GanttEvent) _ganttComposite
						.getEvents()
						.get(_ganttComposite.getEvents().size() - 1),
						SWT.CENTER);
				moveFocus();
			}
		});
		...
		bSelectMidEvent.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				final GanttEvent ge = (GanttEvent) _ganttComposite.getEvents()
						.get(_ganttComposite.getEvents().size() / 2);
		...
		bShowPlanned.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				_ganttComposite.setShowPlannedDates(!_ganttComposite
		...
		bShowDays.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				_ganttComposite.setShowDaysOnEvents(!_ganttComposite
