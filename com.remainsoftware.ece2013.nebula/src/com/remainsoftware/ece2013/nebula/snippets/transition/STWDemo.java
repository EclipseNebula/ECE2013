...
public class STWDemo {
	...
	AbstractSTWDemoFrame[] demoFrames = { new TransitionTest(),
			new TransitionTest1(), new TransitionTest2() };

	static final String[] transitions = { "Fade", "Slide", "Cubic Rotation" };

	public static final String ID = "transitionDemo";
	...
	private Composite createMainComposite(Composite parent) {
		...
		final Combo comboDemo = new Combo(optionsComposite, SWT.DROP_DOWN
				| SWT.READ_ONLY);
		...
		new Label(optionsComposite, SWT.TRANSPARENT)
				.setText("Transition Effect:");
		final Combo comboTransition = new Combo(optionsComposite, SWT.DROP_DOWN
				| SWT.READ_ONLY);
		...
		final Combo comboDirection = new Combo(optionsComposite, SWT.DROP_DOWN
				| SWT.READ_ONLY);
		...
		new Label(optionsComposite, SWT.TRANSPARENT)
				.setText("Total Transition Time (ms):");
		...
		Composite specificOptionsComposite = new Composite(optionsComposite,
				SWT.NONE);
		...
		new Label(fadeOptionsComposite, SWT.TRANSPARENT)
				.setText("Fade Out Start (%):");
		...
		cubicRotationOptionsComposite = new Composite(specificOptionsComposite,
				SWT.NONE);
		cubicRotationOptionsComposite.setLayout(new RowLayout(SWT.VERTICAL));

		new Label(cubicRotationOptionsComposite, SWT.TRANSPARENT)
				.setText("Quality (%):");
		final Spinner spnrQuality = new Spinner(cubicRotationOptionsComposite,
				SWT.NONE);
	...
	private void selectDemo(int index) {
		...
		frameHolderStackLayout.topControl = currentDemo
	...
	private void setQuality(int quality) {
		...
		if (currentTransition instanceof CubicRotationTransition) {
			((CubicRotationTransition) currentTransition)
